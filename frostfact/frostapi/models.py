        ...
        if image_field:
            ...
            if img.format != "PNG":
                if img.mode != "RGBA":
                    ...
                    print(f"Converted image to RGBA mode for PNG format")
                img.save(img_io, format='PNG', optimize=True)
                print(f"Image saved to BytesIO as PNG with optimization")
            else:
                img.save(img_io, format='PNG', optimize=True)
                print(f"PNG image saved to BytesIO with optimization")

            # Generate a safe filename using the instance's slug
            img_name = f"{slugify(instance.slug or 'image')}.png"
            ...
            print(f"Successfully uploaded image to S3: bucket='{bucket_name}', key='{key}', content_type='image/png', ACL='public-read'")
...
class GalleryData(models.Model):
    ...
    gallery_event_type = models.CharField(max_length=100, choices=EventChoices, blank=True, null=True,
                                          verbose_name='Event Type')
